...
if spillTiming.lastSpillDay_serial > spillTiming.endSimDay_serial
  spillTiming.lastSpillDay_serial = spillTiming.endSimDay_serial;
  spillTiming.lastSpillDay_date = spillTiming.endSimDay_date;
end
...
DailySpill = dailyOilParticles(Params.particlesPerBarrel,DS,spillTiming,Params);
...
spillLocation.meanLat       = mean(spillLocation.Lat);
spillLocation.Radius_degLat = mtr2deg(spillLocation.Radius_m, spillLocation.meanLat, 'lat_deg', Dcomp);
spillLocation.Radius_degLon = mtr2deg(spillLocation.Radius_m, spillLocation.meanLat, 'lon_deg', Dcomp);
spillLocation.nSites        = min([numel(spillLocation.Lat);numel(spillLocation.Lon)]); % ADDED
...
% Burning
decay.burn_radiusDegLat  = mtr2deg(decay.burn_radius_m,spillLocation.meanLat,'lat_deg',Dcomp);
decay.burn_radiusDegLon  = mtr2deg(decay.burn_radius_m,spillLocation.meanLat,'lon_deg',Dcomp);
  ...
  for ts = 1:LagrTimeStep.PerDay
    ...
    if any(step_map) && (ismember(0,rem(ts,step_map)) || first_time || final_time)
      [vMap3D,vMap2D,Map_fig3D,Map_fig2D,ensemble_grid] = map_particles(outputFolder,...
        ts,first_time,final_time,spillLocation,Particles,vis_maps,saving,...
    ...
    elseif Params.velocityFieldsType == 3
      [velocities,WindFile,OceanFile] = velocityFields_3(...
        first_time,SerialDay,ts,LagrTimeStep,spillLocation,OceanFile,WindFile,Params);
    elseif Params.velocityFieldsType == 4
      [velocities,WindFile,OceanFile] = velocityFields_4(...
        first_time,SerialDay,ts,LagrTimeStep,spillLocation,OceanFile,WindFile,Params);
    elseif Params.velocityFieldsType == 5
      [velocities,WindFile,OceanFile] = velocityFields_5(...
        first_time,SerialDay,ts,LagrTimeStep,spillLocation,OceanFile,WindFile,Params);
    else
    ...
    if Params.RungeKutta == 4
      Particles = movePartsRK4(OceanFile,velocities,Particles,Params,...
        spillLocation,LagrTimeStep,Dcomp);
    ...
    %---------- Update the age in days of the particles in water ---------%
    Particles.Age_days(Particles.Status == 1) = ...
      Particles.Age_days(Particles.Status == 1) + LagrTimeStep.InDays;
    %----------------------------- Oil decay -----------------------------%
    if decay_true
...
%     if ts == LagrTimeStep.PerDay && SerialDay == spillTiming.endSimDay_serial
      [~,birthMonth,~,~,~,~] = datevec(Particles.birthDate);
      idx_toSave = find(birthMonth>=3 & birthMonth<=6 & Particles.Age_days <= 198);
      Particles_S.Age_days = Particles.Age_days(idx_toSave);
      Particles_S.Status = Particles.Status(idx_toSave);
      Particles_S.Depth = Particles.Depth(idx_toSave);
      Particles_S.Comp = Particles.Comp(idx_toSave);
      Particles_S.Lat = Particles.Lat(idx_toSave);
      Particles_S.Lon = Particles.Lon(idx_toSave);
      Particles_S.Site = Particles.Site(idx_toSave);
      Particles_S.ID = Particles.ID(idx_toSave);
      Particles_S.birthDate = Particles.birthDate(idx_toSave);
      file_name = strrep(dateHour_str,' ','_');
      save([outputFolder.Data,file_name],'Particles_S','-v6')
    end
    %---------------------------------------------------------------------%
  end
end
disp(['Number of particles: ', num2str(numel(Particles.Depth))])
end