...
function [velocities,WindFile,OceanFile] = velocityFields_5(first_time,SerialDay,ts,LagrTimeStep,spillLocation,OceanFile,WindFile,Params)
...
day_DoY_next = day(date_time_next,'dayofyear');
day_DoY_str = num2str(day_DoY);
day_DoY_str_next = num2str(day_DoY_next);
date_str = datestr(date_time,'yyyy-mm-dd');
date_str_next = datestr(date_time_next,'yyyy-mm-dd');
if numel(day_DoY_str) == 1
  day_DoY_str = ['00',day_DoY_str];
elseif numel(day_DoY_str) == 2
  day_DoY_str = ['0',day_DoY_str];
end
if numel(day_DoY_str_next) == 1
  day_DoY_str_next = ['00',day_DoY_str_next];
elseif numel(day_DoY_str_next) == 2
  day_DoY_str_next = ['0',day_DoY_str_next];
end
year_str = datestr(date_time,'yyyy');
year_str_next = datestr(date_time_next,'yyyy');
%-------------------------Get ocean VectorFields--------------------------%
if first_time
    % Ocean file names and variable names
    OceanFile.Prefix        = 'archv.';
    OceanFile.Midfix        = '_';   
    OceanFile.Sufix         = '_00_3z.nc';
    OceanFile.Uname         = 'u';
    OceanFile.Vname         = 'v';
    ...
    % Define variables
    firstFileName = [OceanFile.Prefix,year_str,OceanFile.Midfix,day_DoY_str,OceanFile.Sufix];
    % Reads Lat and Lon
    ...
    readOceanFileT1 = [OceanFile.Prefix,year_str,OceanFile.Midfix,day_DoY_str,OceanFile.Sufix];
    readOceanFileT2 = [OceanFile.Prefix,year_str_next,OceanFile.Midfix,day_DoY_str_next,OceanFile.Sufix];
    ...
    if flag_one ~= flag_two
        ...
        readOceanFile = [OceanFile.Prefix,year_str_next,OceanFile.Midfix,day_DoY_str_next,OceanFile.Sufix];
...
if surface_simulation
    if first_time
        ...
        WindFile.Prefix       = 'UV_wrfout_c3h_d01_';
        WindFile.Midfix       = '_00.';
        WindFile.Sufix        = '.nc';
        WindFile.Uname        = 'Ub';
        WindFile.Vname        = 'Vb';
        WindFile.LatName      = 'XLAT_U';
        WindFile.LonName      = 'XLONG_U';
        % Define variables
        firstFileName = 'wrfout_c_anio_d01_1979-01-02_00:00:00.1979';
        lat_W = double(ncread(firstFileName,WindFile.LatName,[1,1,1],[1,inf,1]));
        lon_W = double(ncread(firstFileName,WindFile.LonName,[1,1,1],[inf,1,1]));
        ...
        readWindFileT1 = [WindFile.Prefix,date_str,WindFile.Midfix,year_str,WindFile.Sufix];
        readWindFileT2 = [WindFile.Prefix,date_str_next,WindFile.Midfix,year_str_next,WindFile.Sufix];
        WindFile.U_T1 = double(ncread(readWindFileT1,WindFile.Uname,...
            [WindFile.Lon_min,WindFile.Lat_min],[WindFile.Lon_numel,WindFile.Lat_numel])');
        WindFile.V_T1 = double(ncread(readWindFileT1,WindFile.Vname,...
            [WindFile.Lon_min,WindFile.Lat_min],[WindFile.Lon_numel,WindFile.Lat_numel])');
        WindFile.U_T2 = double(ncread(readWindFileT2,WindFile.Uname,...
            [WindFile.Lon_min,WindFile.Lat_min],[WindFile.Lon_numel,WindFile.Lat_numel])');
        WindFile.V_T2 = double(ncread(readWindFileT2,WindFile.Vname,...
            [WindFile.Lon_min,WindFile.Lat_min],[WindFile.Lon_numel,WindFile.Lat_numel])');
