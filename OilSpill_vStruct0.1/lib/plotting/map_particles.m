...
%         ensemble_grid.lon = vis_maps.boundaries(1):km2deg(5):vis_maps.boundaries(2);
        ensemble_grid.lat = vis_maps.boundaries(4):-1/4:vis_maps.boundaries(3);
        ensemble_grid.lon = vis_maps.boundaries(1):1/4:vis_maps.boundaries(2);
...
if vis_maps.threeDim
    ...
    % Plot particles
    if vis_maps.colors_BySite_on
      % Plot particles in water couloring by site
      for site_cicle = 1 : spillLocation.nSites
        site_ind = find(Particles.Status == 1 & ...
          Particles.Site == site_cicle);
        plot3(Particles.Lon(site_ind),Particles.Lat(site_ind),...
          -Particles.Depth(site_ind),'.','color',...
          vis_maps.colors_BySite{site_cicle},'MarkerSize',vis_maps.markerSize)
      end
    else
      % Plot particles in water couloring by depth
      for depth_cicle = 1 : spillLocation.n_Depths
    ...
    % Plot spill location
    plot3(repmat(spillLocation.Lon,spillLocation.n_Depths,1),...
        repmat(spillLocation.Lat,spillLocation.n_Depths,1),...
        -repmat(spillLocation.Depths',1,spillLocation.nSites),...
...
if vis_maps.twoDim
    ...
    for depth_cicle = 1:spillLocation.n_Depths
        ...
        % Plot particles
        if vis_maps.colors_BySite_on && depth_cicle
          % Plot particles in water couloring by site
          for site_cicle = 1:spillLocation.nSites
            siteComp_ind = find(Particles.Status == 1 & Particles.Site == site_cicle &...
              Particles.Depth == spillLocation.Depths(depth_cicle));
            plot(Particles.Lon(siteComp_ind),Particles.Lat(siteComp_ind),'.','color',...
              vis_maps.colors_BySite{site_cicle},'MarkerSize',vis_maps.markerSize)
          end
        else
          % Plot particles in water couloring by component
          for comp_cicle = Params.components_number:-1:1
            depthComp_ind = find(Particles.Status == 1 & ...
              Particles.Depth == currentDepth & Particles.Comp == comp_cicle);
...
if final_time && vis_maps.twoDim
    ...
    ylabel('Latitude')
    c_map = hsv(100);
    colormap(c_map(14:end,:))
    hcb = colorbar;
    ylabel(hcb,'Particle positions')
    ...
    hold on
    for depth_cicle = 1:spillLocation.n_Depths
        max_val = max(max(max(ensemble_grid.count(:,:,depth_cicle,:),[],4)))*.2;
        if max_val > 1
          caxis([1,max_val])
%           caxis([1,3000])          
        else
        ...
        for comp_cicle = 1:Params.components_number
          if depth_cicle == spillLocation.n_Depths && comp_cicle == Params.components_number
            if exist(Params.OutputDir,'dir')==0
              mkdir(Params.OutputDir)
            end
            save([Params.OutputDir,'ensemble_grid'],'ensemble_grid')
          end
            img_name = [num2str(spillLocation.Depths(depth_cicle)),...
                ' m, Component ',num2str(comp_cicle)];
            title(img_name)
    ...
    %----------------------------- Coast maps ------------------------------%
    coast_grid.lat = vis_maps.boundaries(4):-1/4:vis_maps.boundaries(3);
    coast_grid.lon = vis_maps.boundaries(1):1/4:vis_maps.boundaries(2);
    ...
    for depth_cicle = 1:spillLocation.n_Depths
        ...
        if depth_cicle == spillLocation.n_Depths
          save([Params.OutputDir,'coast_grid'],'coast_grid')
        end
...
%             caxis([c_min,20])
        ...
        img_name = ['Coast ',num2str(spillLocation.Depths(depth_cicle)),' m'];
        title(img_name)
        % Save image
        if saving.MapsImage_on
            img_name = strrep(img_name,' ','_');
