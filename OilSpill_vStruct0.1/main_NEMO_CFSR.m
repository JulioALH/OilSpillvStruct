...
folderName = {'jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec','jan-dec'};
...
% for sites = 1:6
  for meses = 3
    ...
    spillTiming.startDay_date     = [2011,03,15]; % [2011,03,15];
    spillTiming.lastSpillDay_date = [2011,03,31]; % [2011,03,31];
    spillTiming.endSimDay_date    = [2011,05,12]; % [2011,05,12];
    %---------------------------- Spill location -----------------------------%
    spillLocation.Lat      = sites_lat;
    spillLocation.Lon      = sites_lon;
    spillLocation.Depths   = [1500,  500,  100,     0];
    spillLocation.Radius_m = [1000, 6000, 9000, 10000]; % 2 STD for random initialization of particles
    %------------------------- Local Paths filename --------------------------%
    Params.LocalPaths = 'local_paths_NEMO_CFSR.m';
    %--------------------------- Output directorie ---------------------------%
%     Params.OutputDir = ['/media/julio/My Passport/results_21years/Site',num2str(sites),'/',folderName{meses},'/'];
    Params.OutputDir = ['/media/julio/My Passport/results_NEMO_CFSR/',folderName{meses},'/'];
    %----------------------- Runge-Kutta method: 2 | 4 -----------------------%
    Params.RungeKutta = 4;
    %- Velocity Fields Type: 1(BP)|2(Usumacinta)|3(climatology)|4(NEMO_CFSR) -%
    Params.velocityFieldsType = 4;
    %----------------------------- Model domain ------------------------------%
    Params.domainLimits = [-98, -88, 18.1, 30];
    ...
    decay.expDegrade_Days       = [10, 25, 40, 55];
    ...
    vis_maps.boundaries      = [-98, -88, 18.1, 30, -2500, 0];
    % Isobaths to plot
